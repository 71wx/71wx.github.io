...
Element.implement({
    ...
    zoomImg: function(maxwidth, maxheight, v) {
        ...
        maxwidth = maxwidth || Number.MAX_VALUE;
        maxheight = maxheight || Number.MAX_VALUE;
    ...
    isDisplay: function() {
        return this.isVisible ? this.isVisible() : this.getStyle('display') !== 'none' && (this.offsetWidth + this.offsetHeight) > 0;
    },
    wrapped: function() {
        return new Element('div').adopt(this.clone().setStyle('display',''));
    },
    ...
    outerSize: function() {
        if (!this.isDisplay()) return {x: 0, y: 0};
        return {
...
Element.Events.inputchange = {
    ...
    condition: function(e){
        return supportProperty('onpropertychange') ? e.event.propertyName == 'value' : true;
    }
};
    ...
    Element.implement({
        position: function(options) {
            ...
            if (options.resize) {
                ...
                this.store('resize:position', resizePosition);
